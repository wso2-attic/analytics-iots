...
var waitTime = 1000;
var webSocketURL, trafficStreamWebSocketURL;
...
SpatialObject.prototype.update = function (geoJSON) {
    ...
    this.avgSpeed = geoJSON.properties.avgSpeed;
    this.checkpointsReached = geoJSON.properties.checkpointsReached;
    this.distanceTravelled = geoJSON.properties.distanceTravelled;
    this.durationTravelled = geoJSON.properties.durationTravelled;
    ...
    this.popupTemplate.find('#avgSpeed').html(Math.round(this.avgSpeed * 10) / 10);
    this.popupTemplate.find('#checkpointsReached').html(this.checkpointsReached);
    this.popupTemplate.find('#distanceTravelled').html(Math.round(this.distanceTravelled * 10) / 10);
    this.popupTemplate.find('#durationTravelled').html(Math.round(this.durationTravelled * 10) / 10);
...
var webSocketSend = function (err) {
    if (!websocket.get_opened()) return;
    websocket.send(JSON.stringify({id: "client1"}));
};
...
function intializeWebsocketUrls(){
    ...
    wso2.gadgets.identity.getUsername(function(username) {
        ...
        $.getJSON("/portal/store/carbon.super/fs/gadget/motor-rally/controllers/get_server_info.jag?username="+username, function (data) {

            webSocketURL = 'wss://localhost:'+ data.httpsPort +ApplicationOptions.constance.PATH_SEPARATOR+ApplicationOptions.constance
...
SpatialObject.prototype.stateIcon = function () {
    var iconUrl = "/portal/store/carbon.super/fs/gadget/motor-rally/img/markers/object-types/"  + this.type.toLowerCase();
